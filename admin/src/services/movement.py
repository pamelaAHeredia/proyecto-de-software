...
class MovementService:
    ...
    def insert_movement (self, member_id, amount, type, is_inscription):
        """
            Insersion de un nuevo movimiento en la DB de movimientos

            Arguments:
                member_id (int): Id del miembro al cual se le quiere asignar el movimiento
                amount (float): Monto del movimiento
                type (string): Credito o Debito
                is_inscription (bool): Es para saber si el movimiento sera por una inscripcion o no (solo importante para movimiento de debito)
        """
